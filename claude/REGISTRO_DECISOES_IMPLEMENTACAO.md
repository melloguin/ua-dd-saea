...
### REAL-2.15 RATIFICADA — régua do DDMOP7 definitiva; selo retirado (autor, 2026-08-22)
A fase 2 pooled (D102.3) foi COMPUTADA na validação de fidelidade final sobre 568 células
válidas do corpus congelado (293.944 pontos de ①, `f5/final/reais/DDMOP7.json`):
z* = [1/17 ; 84/690] · znad da nuvem = [1 ; 593/690] · front pooled de 6 pontos, 100%
vindos da busca. **Decisão:** manter `ideal = [0;0]` (ínfimo dedutivo; foi a régua EM
EXECUÇÃO no c122/c262, que a consomem durante a busca — trocar dessincronizaria busca e
análise) e manter `nadir = 468/690` (nunca excedido por ND válido; máx n=383). O z* pooled
não é adotado (estimativa volta a poder ser furada); o nadir do front pooled não é adotado
(cliparia ND válidos). Sensibilidade medida: Spearman(ranking fase-1 × pooled) = 0,9993.
`REGUAS_PROVISORIAS` fica VAZIO; tripwire de teste invertido
(`test_selo_provisorio_vazio_apos_ratificacao`); gate do `accept.py` passa a exigir 0 selos.
**Sanção (2) às 17 células s0 pré-errata** (commit 3240f4b): seus configs não usam a régua
na busca; a métrica é pós-hoc recomputada; c122/c262 são 100% pós-errata (repo_hash 7be66e6).
Verificação: 24/24 em `tests/test_metrics.py`; âncora D92 = 1,0433; 0 furos em 302.643 pontos.

### CORPUS CANÔNICO DECLARADO (autor, 2026-08-22 — item C6 do plano final)
**O corpus canônico do estudo é o M8/T15 em `resultados_experimentos/`, para TODAS as sementes,
inclusive a 42.** A semente 42 da rodada original (auditada em F5 e T11) foi sobrescrita pela
campanha M8 nos configs Python e diverge da atual (c122: 0/25 células com IGD idêntico).
Os laudos F5/T11 continuam válidos como **auditoria de mecanismo** (o mecanismo não mudou —
bit-identidade provada em 5 configs), mas **seus números de métrica não são citáveis como
dado**. O notebook de análise (`cea23a5`) já usa o canônico; este registro formaliza.

### QUARENTENA e81×ESTOQUE40 s22–26 (autor, 2026-08-22 — item A9 do plano final)
Cinco células embrionárias (só ⑥, sem ⑤ e sem parquet; estáticas desde 21/08 17:58) movidas
para `resultados_experimentos/_quarentena/e81_ESTOQUE40_{22..26}/` pelo precedente da semente
30. `LEIA-ME` ao lado. e81×ESTOQUE40 fica com 22 sementes.

### PRIMEIRAS ANÁLISES SÓ COM CÉLULAS A ORÇAMENTO PLENO (autor, 2026-08-22 — item C8, parcial)
Aba `finalizaram_orcamento` em `_contagem_seeds_por_algoritmo_problema.xlsx`: seeds por
algoritmo × problema que avaliaram TODO o orçamento (31D−1) e NÃO foram cortadas pelo teto de
12h — 16.020 de 16.811 (O-21, rodapé como autoridade). Base célula a célula (avaliações, % do
orçamento, tempo) na aba `base_dados`. O destino das 615 células ⚪ teto fica para decisão
posterior; até lá, as primeiras análises usam SÓ as que chegaram ao fim.

### ⑦ DO e103 MATERIALIZADA (autor, 2026-08-22 — item B1 do plano final, opção a)
A camada ⑦ (endpoint oficial do regime offline, DI-08) estava ausente em 733/778 células do
e103 e a análise o EXCLUÍA SILENCIOSAMENTE (758 linhas `erro='final_ausente'`). Gerada por
`scripts/final_eval.py --exp off --alg e103 --problema <P> --all-seeds` sobre um `data_root`
temporário de symlinks (inputs intocados), e copiada para as células canônicas: **703 ⑦ novas**
(25 sintéticos + RE21 + ESTOQUE40), 25 legadas de s42 preservadas. Verificação nas 748:
100 linhas cada; `origem_geracao` = última geração da busca (B7.5) em **748/748**;
`n_nd_pos_real` mediana 28/100. Sidecar DI-08 com `fora_do_orcamento=true`. As 82 "FAIL" do
lote são sementes inexistentes no corpus (13/17/26 etc.), não falhas.
**Pendente:** e103 × DDMOP7 (30 células) exige `matlab.engine` + `DDMOP7.p` — rodar numa VM
quando houver (Processo B, `final_eval` ramo pós-hoc com zona morta em `:230`). Até lá, o
e103 fica declarado fora do endpoint SÓ no DDMOP7 (item B2 do plano).

### DDMOP7 NA APRESENTAÇÃO DOS RESULTADOS (autor, 2026-08-22 — item C14 / D102.8)
O DDMOP7 ENTRA nas tabelas descritivas com asterisco e, no agregado inferencial, estratificado
por classe de operador. Texto do asterisco (pronto): *"DDMOP7: problema de objetivos em grade
(f₁=k/17, f₂=n/690). Sob busca contínua a esparsidade é inalcançável e todos os métodos
colapsam no mesmo front (diagnóstico T15); a codificação de zona morta (τ=0,5; D102.17)
restaurou a busca — 13/15 configs melhoram o sorteio inicial — mas a resolução do front é
estruturalmente baixa (1–6 pontos distintos, mediana 3): o HV separa classes de operador
(p=4,3e-26) e não separa o topo (8 configs em 0,6%). Leia-o como comparação de resolução
declaradamente baixa; a história metodológica completa está em handoff/T15-*."*
