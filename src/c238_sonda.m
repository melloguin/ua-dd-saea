...
% Chamado do embrulho N.5 (EIM.m:109) logo APOS o fit dos M kriging e ANTES de
...
% transf_params, c238_instrument.m:89-101) e deliberada e vai declarada no
...
% O c238 re-escala Y por MIN-MAX A CADA ITERACAO (EIM.m:67-80), entao o (u,s) que
...
% (espaco_modelo="transformado" + C3, c238_instrument.m:89-101); a SONDA, nao — ela
...
%       mu_cru    = u .* yrange + ymin        (inversa exata de EIM.m:80)
...
% `ymin` e o PRE-guard (EIM.m:67); `yrange` e o POS-guard max(range,eps) (EIM.m:79)
% — o MESMO par que a busca serializa em c238_instrument.m:89. Nao trocar a ordem:
% sob objetivo constante (guard_range, EIM.m:71-78) yrange vira eps, e usar o
...
% GP_Train.m:35-36 a partir dos lb/ub do Problem (EIM.m:96) = bounds nativos = o
% espaco do artefato da sonda. Sem sort, sem unique, sem clamp (I3: o join com o
% gabarito e POR POSICAO).
%
% Por que ESTE ponto (EIM.m:109, entre :99 e :112) — as 4 condicoes:
%   1. POS-FIT COMPLETO: os M GP_Train do laco :95-98 terminaram e `tempo_fit`
%      fechou em :99; `GP_obj` e EXATAMENTE o modelo que a busca usa em :113-114;
%   2. PRE-1a DECISAO: a 1a decisao e o Optimizer_GA (:113) — entre :109 e :113 nao
%      ha nada alem dos comentarios :110-111 e do `t1 = tic` :112;
...
%      EIM.m:97). O 1o consumidor da iteracao e Optimizer_GA.m:9 (lhsdesign),
%      depois do hook. O save/restore do RNG acontece assim mesmo (SondaState.m:196-197);
%   4. PRE-Problem.Evaluation: a unica avaliacao real do ciclo e EIM.m:127.
%   + I6: :109 fica APOS `toc(t0)` (:99) e ANTES de `t1 = tic` (:112) — e o UNICO
...
% mas SondaState.event so emite meta.modelo — SondaState.m:233/:240). Os theta/lnL/
% sigma2/n do kriging vao em `modelo_hp` na linha `c238_gen` (S.7.1), nao aqui.
    ...
    % O treino do c238 e a Population INTEIRA (train_x = sample_x(ia,:), EIM.m:87-89,
    % sample_x = Population.decs, :63) — NAO ha subamostragem. A unica reducao e o
    % dedup bit-exato `unique(...,'stable')` (:87, guard do chol de GP_Train.m:22), e
    ...
    % deste ciclo (EIM.m:127) => o ftm do c238 e MONOTONICO (+1 por iteracao).
