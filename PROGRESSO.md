...
- **⬜ Interlúdio — FIX das 2 pontas soltas do c217** (instrumentação objetiva, não fidelidade): `c217_instrument.m` grava `n_contradicoes==n_empates` (só um valor computado, linha 37/71) e `pred_confianca=Error1` (sempre p⁺, mesmo no estado uso-reverso que deveria ser p⁻=Error2). O autor pediu para fechar isso antes de continuar o fan-out.
