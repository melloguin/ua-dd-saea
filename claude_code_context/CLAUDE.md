...
**Leia SOMENTE:** (1) este CLAUDE.md; (2) a pasta `00_fundacao/` (uma vez, na primeira sessão / Fase 0); (3) o `00_contrato_*.md` da rodada atual; (4) o `alg_*.md` do ÚNICO algoritmo desta sessão. **NÃO leia** os `alg_*.md` de outros algoritmos, outras rodadas, nem a SPEC inteira — cada patch de fidelidade pertence a UM algoritmo (aplicar o dropout do e7 ou o kernel do c262 em outro lugar é o erro clássico). A SPEC completa existe para consulta pontual (grep de uma seção citada), nunca para leitura integral.
