...
function [status, info] = run_piso(alg, problema, semente, exp, dataRoot)
    ...
    % [BL-13/T14.6] A `geracoes_derivadas` RAMIFICA POR FAMILIA DE OPERADOR.
    % A string unica declarava o mecanismo do NSGA-II para os 4 pisos, mas
    % `MOEAD.m:45` e `SMSEMOA.m:29` chamam `OperatorGAhalf` — 1 prole por
    % subproblema, N_efetivo por geracao —, enquanto `NSGAII.m:27` e
    % `NSGAIII.m:29` chamam `OperatorGA` — prole em PARES, 2*floor(N_ef/2).
    % Os denominadores so DIVERGEM com N_efetivo IMPAR, isto e, nas celulas
    % M=3 do lattice (N_ef=15): e exatamente ali que o moead errava.
    % MEDIDO nas 112 celulas da s42 (previsto x `n_geracoes` real):
    %   formula unica de hoje ... nsga2 27/28 · nsga3 27/28 · moead 21/28 · smsemoa 28/28 = 103/112
    %   ramificada ............... nsga2 27/28 · nsga3 27/28 · moead 28/28 · smsemoa 28/28 = 110/112
    % ⚠ O bloqueador BL-13 prescrevia `floor((20D+clones)/N_ef) + 1` para o
    % moead: o "+1" acerta 0/28. Sem ele, 28/28. Prescricao corrigida pelo dado.
    if any(strcmp(char(alg), {'moead','smsemoa'}))
        familia_op  = "OperatorGAhalf";
        prole_txt   = "1 prole por subproblema — N_efetivo por geracao";
        denom_txt   = "N_efetivo";
        sitio_txt   = "MOEAD.m:45 / SMSEMOA.m:29";
        acerto_txt  = "28/28 em cada um dos dois";
    else
        familia_op  = "OperatorGA";
        prole_txt   = "prole em PARES — 2*floor(N_efetivo/2) por geracao (MatingPool)";
        denom_txt   = "2*floor(N_efetivo/2)";
        sitio_txt   = "NSGAII.m:27 / NSGAIII.m:29";
        acerto_txt  = "27/28 em cada um dos dois";
    end
    geracoes_derivadas_txt = "EMERGENTE, nao fechada [I-13/A24 corrigido " + ...
        "contra o dado em 2026-07-30; RAMIFICADA POR OPERADOR em 2026-08-01, " + ...
        "BL-13]. O laco roda ate o hard-stop (31D-1 FE EXATAS) e cada duplicata " + ...
        "bit-a-bit (cache-hit D89, 0 FE) ADIA o fim. Este config usa " + ...
        familia_op + " (" + sitio_txt + "): " + prole_txt + ". Melhor " + ...
        "aproximacao MEDIDA: floor((20D + n_dup) / " + denom_txt + "), que " + ...
        "acerta " + acerto_txt + " nas celulas da s42 — 110/112 somando os 4 " + ...
        "pisos, contra 103/112 da formula unica que valia para todos. As que " + ...
        "restam erram por 1 porque o efeito de um cache-hit depende de QUANDO " + ...
        "ele cai, nao so de quantos sao. ⚠ Os dois denominadores so divergem " + ...
        "com N_efetivo IMPAR (as celulas M=3 do lattice, N_ef=15) — em N_ef=20 " + ...
        "sao o MESMO numero, e e por isso que o defeito so aparecia no moead. " + ...
        "O valor REAL de cada run esta em `n_geracoes`, neste mesmo manifesto " + ...
        "— nao derive, LEIA.";
    operadores_txt = "Balde C: " + familia_op + " stock (SBX proC=1 dis_c=20; " + ...
        "PM proM=1 dis_m=20)";
    man.params = struct( ...
