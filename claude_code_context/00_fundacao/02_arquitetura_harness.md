...
- **Python — ambientes isolados (v3.0.1: ~6 + ponte):**
  ...
  4. **c149 LBN-MOBO** *(⚠ o harness JÁ EXISTE — REUSE `standalone_harness`: `pin_runtime`/`env_info` · `load_doe`+`FEBudget` · `seed_base`/`iteration_seed` · `preserve_all_rng`/`guarded_pymoo_minimize` · `iteration_cleanup` (D86 — crítico no ensemble K=10) · `load_sonda`/`sonda_due`/`emit_sonda_block` · `minimo_comum_di10` · `SnapshotBuffer` · `write_run_outputs`; esqueleto copiável = `run_stubr3`. NÃO reimplemente — gancho faltando ⇒ pára-e-pergunta D81)* [E-01/DI-16.15] — torch + pymoo 0.6.x (notebook → script; E.3).
