...
def arg_parser():
    ...
    #Task_ID for job array
    parser.add_argument("--task_id", type=int, required=True, help="Job array ID")
...
val=args.task_id #job array ID
print("Sent val is: ",val)
...
from botorch.test_functions.multi_objective import (
    ...
    ZDT2,ZDT3, DiscBrake, MW7, OSY, WeldedBeam,ZDT1,DTLZ4,DTLZ5
)
...
if func_sent=="branincurrin":
    ...
    problem = ZDT1(negate=True,dim=args["dim"]).to(device=device, dtype=dtype)
elif func_sent=="zdt2":
...
print(f"Repetition {val}:",flush=True)
...
if acquisition_function == "qpots":
    ...
    for iter in range(args["iters"]):
            ...
            for i in range(xnew_size):
                ...
                if tc is not None: #tc is None when R is invertible
                    if torch.abs(tc) > torch.tensor(args["threshold"]): #Perform partial eval at x when total correlation above given threshold
                        ...
                        fx = f(unnormalize(x_new[i], bounds)).view(-1).to(y_new.dtype)
                        y_new[i, S] = fx[S]
        ...
        if args["mt"]==1:
            ...
            tag="Model_list"
        file_saving_inloop(func_sent=func_sent,tag=tag,train_X_full=train_X_full,train_Y_full=train_Y_full,hvs=hvs,true_hvs=true_hvs,times=times,REP=val,coupled_y=True_coupled_train_y)
        np.save(f"{args['wd']}/{val}_{func_sent}_{tag}_exception_handling_NSGA.npy", NSGA_expansion_counter)
        np.save(f"{args['wd']}/{val}_{func_sent}_{tag}_Non_invertible_counter.npy", Non_invertible_counter)
...
### HVKG ###
    ...
    for iter in range(args["iters"]):
        ...
        file_saving_inloop(func_sent=func_sent,tag=tag,train_X_full=train_X_full,train_Y_full=train_obj_hvkg_list,hvs=hvs_hvkg,true_hvs=true_hvs,times=times,REP=val,coupled_y=True_coupled_train_y)
...
#qNEHVI        
    ...
    for iter in range(args["iters"]):
        ...
        file_saving_inloop(func_sent=func_sent,tag=tag,train_X_full=train_X_full,train_Y_full=train_obj_qnehvi_list,hvs=hvs_qnehvi,true_hvs=true_hvs,times=times,REP=val,coupled_y=True_coupled_train_y)
    ...
    for iter in range(args["iters"]):
        ...
        file_saving_inloop(func_sent=func_sent,tag=tag,train_X_full=train_X_full,train_Y_full=train_obj_random_list,hvs=hvs_random,true_hvs=true_hvs,times=times,REP=val,coupled_y=True_coupled_train_y)
#Sobol Decoupled
...
## Saving dummy file to show that this task finished its job:
done_file = f"done_{val}"
with open(done_file, "w") as f:
    f.write("done")
print(f"Task {val} wrote {done_file}",flush=True)


#Merging all into master file
if val == 0:

    print("Task 0 waiting for all done files...",flush=True)
    expected_done_files = [f"done_{i}" for i in range(REPS)]

    while True:
        if all(os.path.exists(f) for f in expected_done_files):
            break
        time.sleep(2) #Sleeping for 2 seconds, can change this to any value
