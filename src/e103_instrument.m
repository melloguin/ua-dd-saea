    ...
    switch fase
        ...
        case 'gen'
            ...
            for i = 1:nsel
                ...
                % [BL-18] `espaco_modelo` = "cru" TAMBEM na busca. As linhas de
                % SONDA do mesmo run ja o declaravam (e103_sonda.m:114,138), com
                % os MESMOS modelos sobre o MESMO X nativo — a assimetria
                % INTRA-⑤ e o que fazia disto defeito e nao escolha (DEF-C3 do
                % CONTRATO §3 manda declarar o espaco). Medido na s42: sonda
                % 40.000 linhas com "cru" x busca 19.800 com NULL, em 45/45.
        ...
        case 'busca_fim'
            ...
            % [I-7/A5] `tempo_geracao_s` — o e103 era o UNICO config sem o campo
            % na ④, o que o deixava FORA da comparacao de custo por fase do §17.6
            % (a ⑤ agregada nao substitui: a curva compara fit x busca x geracao
            % ENTRE configs). Aqui a ④ tem 1 linha por desenho (treino UNICO,
            % D-09/T-8), entao `tempo_geracao_s` = fit + busca da unica "geracao"
            % que existe — e SEM a sonda, como manda a DI-13.10. No e103 nao ha
            % desconto a fazer: a sonda offline dispara NO setup, ANTES do
            % t0_busca (IBEAMS.m), logo `tempo_busca` ja nasce limpo dela.
            timing = struct('n_acumulado', Global.n_dataset, ...
                            'tempo_fit_s', tempo_krig + tempo_rbfn, ...
                            'tempo_busca_s', tempo_busca, ...
                            'tempo_geracao_s', (tempo_krig + tempo_rbfn) + tempo_busca, ...
