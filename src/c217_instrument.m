...
% desta iteracao (NotTerminated), entao ② (hook) e ③/timing (aqui) partilham o mesmo g.
    ...
    if lote > 0
        ...
        for i = 1:lote
            ...
            % [BL-18] `espaco_modelo` = "cru" TAMBEM na busca — MESMA assimetria
            % do e103, medida na s42: a sonda deste config ja declarava "cru"
            % (c217_sonda.m:79) para o MESMO "PNN-par" sobre o MESMO X nativo,
            % e as linhas de busca saiam NULL (198.000 x 201 numa celula).
