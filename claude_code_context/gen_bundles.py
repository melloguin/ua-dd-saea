...
write('10_rodada1_matlab/00_contrato_rodada1.md', 'Rodada 1 — contrato transversal MATLAB/PlatEMO (Mac)',
      ...
      epigraph=("**Decisões-chave desta rodada (Anexo D §D.3):** D59 (`parfor` canônico; `rng(seed)` DEPOIS do Problem, probe isolado) · "
...
ALGS = {
 ...
 'alg_c217_pcsaea': ('c217 PC-SAEA (PlatEMO 4.15) — caso-modelo da auditoria', ['c217'], 'c217 PC-SAEA', r'I\.5 ', r'L\.5 ', r'M\.4 ', None, 'c217 ',
                     "Patch = **2 guardas** (D17); N=50; **referência de fidelidade (validação MANUAL do autor — D97): IGD≈6,9212e-2, faixa-guia ±3σ=±2,335e-2** (não é limiar automático; o antigo ±1σ reprovaria 1/3 das corretas); log `.txt` por geração (§17.2.1)."),
...
write('20_rodada2_botorch/00_contrato_rodada2.md', 'Rodada 2 — contrato transversal BoTorch (Python, Vertex AI)',
      ...
      epigraph=("**Decisões-chave:** BoTorch **OFICIAL 0.18.1** (nunca o fork do device — N.2.3) · D62 (`SeedSequence((base,alg_id,iter,uso_id))` p/ toda semente interna) · "
...
write('30_rodada3_standalone/00_contrato_rodada3.md', 'Rodada 3 — contrato transversal standalone (Python; offline + BO de autor)',
      ...
      epigraph=("**Decisões-chave:** D77 (**piso offline = DESDEO mode 12, GP-média = b5-sem-σ**; a listagem PlatEMO morreu) · D56 (b5 = 2 configs `b5r`/`b5m`) · "
